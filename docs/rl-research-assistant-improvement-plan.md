...
文档状态：分阶段实施中。第 18 节的首个 M0/R01 切片已实现项目、资料快照、论文版本、证据、待核验主张与报告 API；使用方式和验收范围见 [科研基础说明](rl-research-foundation.md)。其余模块仍是后续设计，不能视为已交付能力。

首个研究主线已由用户选定为 **PPO 单智能体复现与实验分析**。当前样例只包含公开论文元数据、摘要短摘录与合成指标，训练、实验协议管理和统计分析尚未执行或实现。
...
- 科研主线选“复现一个已发表结果，再验证一个单因素假设”。正式研究环境、基线和规模在 M0 固定。
...
| 文献检索 | [research.py](../app/tools/builtin/research.py) 中的 arXiv 检索、时间与来源信息 | 结构化返回、分页、版本去重、全文导入、查询记录与缓存；当前呈现文本会截短摘要 |
...
建议先执行 **M0 + R01**，目标是一周内交付一个很小但完整的科研项目骨架：
