...
- A fresh fetch succeeded and revealed that `origin/main` contains a much newer execution-workbench series than the feature branch. Logged the overlap and integration rule: retain newer `main` behavior for conflicting Web/API/test files rather than overwrite it with the stale branch implementation.
